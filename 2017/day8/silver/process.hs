...
solve :: [[String]] -> Map.Map String Register -> Register
solve inputStrings mIn 
  | length inputStrings == 0 = sortRegisters mIn
  | otherwise = solve carryOver (executeInstruction instruction newM)
  where 
...
sortRegisters :: Map.Map String Register -> Register
sortRegisters mIn = last $ List.sort $ map snd (Map.toList mIn)
